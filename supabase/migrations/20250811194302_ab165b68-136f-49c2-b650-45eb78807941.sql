-- Secure SELECT access for ea_intake_submissions and add role-based access (fixed pg_policies checks)
...
-- Minimal policy so users can see their own roles
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own roles' AND schemaname = 'public' AND tablename = 'user_roles'
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'ea_intake_submissions' 
      AND policyname = 'Admins can view EA intake submissions'
  ) THEN
